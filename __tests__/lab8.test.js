describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let prodItem = await page.$('product-item');
	  ...
	  let removeButton = await shadow.$('button');
	  await removeButton.click();
	  let removeText = await removeButton.getProperty('innerText');
	  let textValue = await removeText.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let textValue = await countText.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){
      shadow = await prodItems[i].getProperty('shadowRoot');
      let addToCartButton = await shadow.$('button');
      let addToCartText = await addToCartButton.getProperty('innerText');
      let textValue = await addToCartText.jsonValue();
      expect(textValue).toBe("Remove from Cart");
    }
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const items = await page.evaluate(() => localStorage.getItem("cart"));
